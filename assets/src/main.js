...
function createOverlayWindow() {
  ...
  overlayWin.webContents.session.webRequest.onHeadersReceived({ urls: ['file://*/*'] }, (details, callback) => {
    callback({ responseHeaders: { ...details.responseHeaders, 'Content-Security-Policy': ["default-src 'self' 'unsafe-inline' 'unsafe-eval'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https:; connect-src https: wss:; frame-src *; img-src * data:;"] } });
  });
...
function openUbiAuthWindow() {
  return new Promise((resolve, reject) => {
    ...
    // Track requestIds of Ubisoft API calls so we can grab body on loadingFinished
    const pendingUbiRequests = new Map();
    ...
    ubiAuthWin.webContents.debugger.on("message", (_e, method, params) => {
      if (method === "Network.responseReceived") {
        ...
        if (!isUbiApi) return;
        console.log("[Auth CDP] Response:", status, url);
        // Store for loadingFinished
        pendingUbiRequests.set(params.requestId, url);
      }

      // loadingFinished fires AFTER body is fully available — more reliable than responseReceived
      if (method === "Network.loadingFinished") {
        const url = pendingUbiRequests.get(params.requestId);
        if (!url) return;
        pendingUbiRequests.delete(params.requestId);
        console.log("[Auth CDP] Loading finished, grabbing body for:", url);
        ;(async () => {
          try {
            ...
            if (!result?.body) { console.log("[Auth CDP] No body for:", url); return; }
            let json;
            try { json = JSON.parse(result.body); } catch { return; }
            console.log("[Auth CDP] Body keys:", Object.keys(json).join(","), "for:", url);
            if (json?.ticket) {
    ...
    const checkCookies = async () => {
      ...
      try {
        ...
        if (tokenCookie) {
          console.log("[Auth] Cookie found:", tokenCookie.name, "— exchanging");
